...
export class Reactor<V> implements Observer {
    ...
    protected constructor(
         ...
         * @internal
         */
        private readonly _errorHandler: (error: unknown) => void,
         ...
         */
        private readonly afterShutdown?: () => void,
    ) {}
     ...
     * @param options lifecycle options
     * @param afterShutdown an optional callback that fires when the reactor is stopped indefinitely (by once or until option)
     */
    static create<W>(
        ...
        const reactor = new Reactor<W>(controlledDerivable, errorHandler, reaction, options?.afterShutdown);
