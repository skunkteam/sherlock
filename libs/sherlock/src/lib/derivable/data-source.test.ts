...
describe('derivable/data-source', () => {
    ...
    class SimpleDataSource<V> extends PullDataSource<V> {
        constructor(private _value: MaybeFinalState<V>) {
            super();
        }
    ...
    describe('(simple)', () => {
        testDerivable(
            a$ => new SimpleDataSource(a$.getMaybeFinalState()),
            'settable',
    ...
    describe('(derived)', () => {
        testDerivable(
            <V>(a$: Derivable<V>) =>
                new SimpleDataSource(a$.map(val => ({ val })).getMaybeFinalState()).derive(obj => obj.val),
            'no-error-augmentation',
            'no-rollback-support',
        );
    });
    describe('(mapped)', () => {
        testDerivable(
            <V>(a$: Derivable<V>) =>
                new SimpleDataSource(a$.map(val => ({ val })).getMaybeFinalState()).map(obj => obj.val),
            'no-error-augmentation',
            'no-rollback-support',
        );
    });
    describe('(bi-mapped)', () => {
        testDerivable(
            <V>(a$: Derivable<V>) =>
                new SimpleDataSource(a$.map(val => ({ val })).getMaybeFinalState()).map(
                    obj => obj.val,
                    val => ({ val }),
                ),
            'settable',
    ...
    describe('when using other derivables', () => {
        it('should not polute outer derivation dependency administration', () => {
            ...
            const d$ = b$.derive(v => v).autoCache() as Derivation<string>;
            d$.get();
