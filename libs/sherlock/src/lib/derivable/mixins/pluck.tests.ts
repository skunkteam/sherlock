...
export function testPluck(factories: Factories, isSettable: boolean, isAtom: boolean) {
    describe('#pluck', () => {
        ...
        if (isSettable) {
            ...
            beforeEach(() => {
                value$ = assertSettable(factories.value(new MyClass('value')));
            });

            describe('(lensed with constant key)', () => {
                let plucked$: SettableDerivable<string>;
                beforeEach(() => {
                    plucked$ = value$.pluck('key');
                });
                ...
                it('should produce a lens that can change immutable values', () => {
                    const immValue$ = factories.value(
                        fromJS({
                            a: ['value1', 'value2'],
                            b: ['value3', 'value4'],
                        }),
                ...
                it('should not allow readonly immutable values to be changed', () => {
                    const immValue$ = factories.value(Seq.Indexed.of(1, 2, 3)) as Atom<unknown>;
            ...
            describe('(lensed with derivable key)', () => {
                ...
                let plucked$: SettableDerivable<unknown>;
                beforeEach(() => {
                    key$ = atom('key');
                });
                beforeEach(() => {
