...
describe('derivable/map', () => {
    ...
    describe('(bi-mapping)', () => {
        testDerivable(
            <V>(a$: Atom<V>) => {
                ...
                return new Atom(startValue).map(
                    obj => obj.val,
                    val => ({ val }),
                );
            },
    ...
    it('should cache thrown errors to rethrow them on multiple accesses until the derivation produces a new result', () => {
        ...
        const deriver = jest.fn((a: boolean) => {
            if (a) {
                throw theError;
            } else {
                return 'a value';
    ...
    it('should use the Mapping object as `this`', () => {
        ...
        const mapping1$ = base$.map(function (this: any) {
            expect(this).toBe(mapping1$);
            return 1;
        });
        const mapping2$ = base$.mapState(function (this: any) {
            expect(this).toBe(mapping2$);
            return 2;
        });
