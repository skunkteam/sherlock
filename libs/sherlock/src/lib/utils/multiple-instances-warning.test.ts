...
describe('util/multiple-instances-warning', () => {
    ...
    const globalObject = global as { [sherlockInstances]?: number };

    beforeEach(() => {
        delete globalObject[sherlockInstances];
    });

    beforeEach(() => {
        consoleWarnStub = jest.spyOn(console, 'warn').mockReturnValue();
    });

    it('should add sherlockInstanceCount to global object', () => {
        runGlobalStateWarning();
        expect(globalObject[sherlockInstances]).toBe(1);
    ...
    it('should call console.warn when more than one instance of sherlock is loaded', () => {
        runGlobalStateWarning();
        runGlobalStateWarning();
        expect(globalObject[sherlockInstances]).toBe(2);
        expect(consoleWarnStub).toHaveBeenCalledWith(
            '2 instances of Sherlock detected. This could cause unexpected results.',
        );
    });
});
