...
describe('util/config', () => {
    describe('#equals', () => {
        ...
        beforeEach(() => {
            config.equals = jest.fn(() => returnValue);
        });
        afterEach(() => {
            config.equals = defaultEquals;
        });
    ...
    describe('#plucker', () => {
         ...
         */
        class MyWrapper {
            constructor(readonly value: unknown, readonly property?: string | number) {}
        }
        ...
        it('should allow an arbitrary plucker implementation', () => {
                ...
                set(newValue, oldObject, key) {
                    if (oldObject instanceof MyWrapper) {
                        ...
                        return new MyWrapper(
                            defaultPlucker.set.call(this, newValue.value, oldObject.value, key),
                            oldObject.property,
                        );
                    }
            ...
            expect(a$.pluck('deeply').get()).toHaveProperty('value', {
                nested: 'value',
            });

            const inner = a$.pluck('deeply').pluck('nested');
            expect(inner.get()).toBeInstanceOf(MyWrapper);
            expect(inner.get()).toHaveProperty('property', 'nested');
            expect(inner.get()).toHaveProperty('value', 'value');
