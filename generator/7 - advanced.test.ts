...
describe('advanced', () => {
    ...
    it('`constant`', () => {
        ...
        // Remove this after taking your turn below. // #QUESTION
        expect(false).toBe(true); // #QUESTION
        // .toThrow() or .not.toThrow()? ↴ (2x)
        expect(() => c.get()) /* __YOUR_TURN__ */; // #QUESTION
        expect(() => c.set('new value')) /* __YOUR_TURN__ */; // #QUESTION
        expect(() => c.get()).not.toThrow(); /* __YOUR_TURN__ */ // #ANSWER
        expect(() => c.set('new value')).toThrow() /* __YOUR_TURN__ */; // #ANSWER
    });
    ...
    it('`templates`', () => {
        ...
        expect(myDerivable.get()).toBe(__YOUR_TURN__) /* __YOUR_TURN__ */; // #QUESTION
        expect(myDerivable.get()).toBe(`I want to go to 1 party`); // #ANSWER
    });
    ...
    it('`.swap()`', () => {
        ...
        myCounter$.set(plusOne(myCounter$.get())); // #QUESTION
        myCounter$.swap(plusOne); // #ANSWER
    ...
    it('`.take()`', () => {
         ...
         * when it is `allowed`.
         */
        const myLimitedAtom$ = myAtom$.take(__YOUR_TURN__); // #QUESTION
        const myLimitedAtom$ = myAtom$.take({ when: parent$ => parent$.is('allowed') }); // #ANSWER
    ...
    describe('`.value`', () => {
        ...
        it('as a getter/setter', () => {
             ...
             */
            expect(__YOUR_TURN__).toEqual('foo'); // #QUESTION
            expect(myAtom$.value).toEqual('foo'); // #ANSWER
            /**
             * ** Your Turn **
             *
             * Now use the `.value` accessor to set a 'new value'.
             */
            myAtom$.value = __YOUR_TURN__; // #QUESTION
            myAtom$.value = 'new value'; // #ANSWER
        ...
        it('will not throw when `unresolved`', () => {
             ...
             * ** Your Turn **
             */
            expect(myAtom$.value).toEqual(__YOUR_TURN__); // #QUESTION
            expect(myAtom$.value).toEqual(undefined); // #ANSWER
        });
    ...
    describe('`.map()`', () => {
        ...
        it('triggers when the source changes', () => {
             ...
             */
            const mappedAtom$: Derivable<string> = __YOUR_TURN__; // #QUESTION
            const mappedAtom$: Derivable<string> = myAtom$.map(value => value.toString().repeat(value)); // #ANSWER
        ...
        it('similar to `map()` on arrays', () => {
            ...
            expect(myMappedList).toMatchObject(__YOUR_TURN__); // #QUESTION
            expect(myMappedList).toMatchObject([2]); // #ANSWER

            const myAtom$ = atom(1);
            let myMappedDerivable$ = myAtom$.map(addOne);
            expect(myMappedDerivable$.value).toBe(__YOUR_TURN__); // #QUESTION
            expect(myMappedDerivable$.value).toBe(2); // #ANSWER

            // You can combine them too.
            const myAtom2$ = atom([1]);
            const myMappedDerivable2$ = myAtom2$.map(v => v.map(addOne));
            expect(myMappedDerivable2$.value).toMatchObject(__YOUR_TURN__); // #QUESTION
            expect(myMappedDerivable2$.value).toMatchObject([2]); // #ANSWER
        });
        ...
        it('one-way flow', () => {
            ...
            expect(myMappedAtom$.value).toBe(__YOUR_TURN__); // #QUESTION
            expect(myAtom$.value).toBe(5); // #ANSWER
            expect(myMappedAtom$.value).toBe(6); // #ANSWER

            // ...but this may seem weird.
            myMappedAtom$.set(5);
            expect(myAtom$.value).toBe(__YOUR_TURN__); // #QUESTION
            expect(myMappedAtom$.value).toBe(__YOUR_TURN__); // #QUESTION
            expect(myAtom$.value).toBe(10); // #ANSWER
            expect(myMappedAtom$.value).toBe(11); // #ANSWER
        ...
        it('`.flatMap()`', () => {
            ...
            myMappedAtom$ = __YOUR_TURN__ as Derivable<number>; // #QUESTION
            myMappedAtom$ = myAtom$.flatMap(atomize) as Derivable<number>; // #ANSWER
            ...
            const myFlatMappedList = __YOUR_TURN__; // #QUESTION
            const myFlatMappedList = myList.flatMap(v => [v, v + 1]); // #ANSWER
    ...
    describe('States', () => {
        ...
        it('value states, unresolved states, and error states', () => {
            ...
            expect(myAtom$.getState()).toBe(1); // #ANSWER
             ...
             */
            expect(myAtom$.getState()).toBe(__YOUR_TURN__); // #QUESTION
            expect(myAtom$.getState()).toBe(unresolved); // #ANSWER
             ...
             * What do you expect the state to be?
             */
            expect(myAtom$.getState()).toBeInstanceOf(__YOUR_TURN__); // #QUESTION
            expect(myAtom$.getState()).toBeInstanceOf(ErrorWrapper); // #ANSWER
            ...
            myAtom$.set(1);
            expect(stateToString(myAtom$.getState())).toBe(__YOUR_TURN__); // #QUESTION
            expect(stateToString(myAtom$.getState())).toBe('2'); // #ANSWER

            myAtom$.unset();
            expect(stateToString(myAtom$.getState())).toBe(__YOUR_TURN__); // #QUESTION
            expect(stateToString(myAtom$.getState())).toBe('Symbol(unresolved)'); // #ANSWER

            myAtom$.setError('OH NO!');
            expect(stateToString(myAtom$.getState())).toBe(__YOUR_TURN__); // #QUESTION
            expect(stateToString(myAtom$.getState())).toBe('OH NO!'); // #ANSWER
        });
        ...
        it('`.mapState()`', () => {
            ...
            myAtom$.set(2);
            expect(myAtom$.value).toBe(__YOUR_TURN__); // #QUESTION
            expect(myAtom$.value).toBe(2); // #ANSWER
            expect(myMappedAtom$.value).toBe(__YOUR_TURN__); // #QUESTION
            expect(myMappedAtom$.value).toBe(2); // #ANSWER

            myAtom$.unset();
            expect(myAtom$.value).toBe(__YOUR_TURN__); // #QUESTION
            expect(myAtom$.value).toBe(undefined); // #ANSWER
            expect(myMappedAtom$.value).toBe(__YOUR_TURN__); // #QUESTION
            expect(myMappedAtom$.value).toBe(0); // #ANSWER
    ...
    describe('`.pluck()`', () => {
        ...
        beforeEach(() => {
             ...
             */
            firstProp$ = __YOUR_TURN__; // #QUESTION
            firstProp$ = myMap$.pluck('firstProp') as SettableDerivable<string>; // #ANSWER
        });
        ...
        it('can be used as a normal `Derivable`', () => {
             ...
             */
            expect(firstProp$.get()).toEqual(__YOUR_TURN__); // #QUESTION
            expect(firstProp$.get()).toEqual('firstValue'); // #ANSWER

            // the plucked `Derivable` should be settable
            firstProp$.set('other value');
            // is the `Derivable` value the same as was set?
            expect(firstProp$.get()).toEqual(__YOUR_TURN__); // #QUESTION
            expect(firstProp$.get()).toEqual('other value'); // #ANSWER

            // How many times was the spy called? Note the `skipFirst`..
            expect(reactPropSpy).toHaveBeenCalledTimes(__YOUR_TURN__); // #QUESTION
            expect(reactPropSpy).toHaveBeenCalledTimes(1); // #ANSWER

            // ...and what was the value?
            expect(reactPropSpy).toHaveBeenLastCalledWith(__YOUR_TURN__, expect.toBeFunction()); // #QUESTION
            expect(reactPropSpy).toHaveBeenLastCalledWith('other value', expect.toBeFunction()); // #ANSWER
        });
        ...
        it('will react to changes in the source `Derivable`', () => {
            ...
            // How many times was the spy called? Note the `skipFirst`.
            expect(reactPropSpy).toHaveBeenCalledTimes(__YOUR_TURN__); // #QUESTION
            expect(reactPropSpy).toHaveBeenCalledTimes(0); // #ANSWER
            ...
            // ...and what was the value?
            expect(reactPropSpy).toHaveBeenLastCalledWith(__YOUR_TURN__, expect.toBeFunction()); // #QUESTION
            expect(reactPropSpy).toHaveBeenLastCalledWith('new value', expect.toBeFunction()); // #ANSWER
        });
